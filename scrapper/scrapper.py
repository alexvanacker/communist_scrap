...
def scrap_all_articles(url_folder, articles_folder):
    """ Will scrap all URLs from the pickle files found in the URL folder.
    
    url_folder -- Path to the folder containing pickle files, generated by the crawler.
    
    """
    pickle_files = [ f for f in os.listdir(url_folder) if f.endswith('.p')]
    
    start = time.time()
    for f in pickle_files:
        pickle_filepath = osp.join(url_folder, f)
        # Read the URLs
        urls = pickle.load(open(pickle_filepath, 'rb'))
        ...
        for url in urls:
            ...
            write_raw_infos(real_url, articles_folder)
